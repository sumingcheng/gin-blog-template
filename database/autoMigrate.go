...
func createInitialUsers(db *gorm.DB) error {
	...
	if count == 0 {
		users := []User{
			...
			{Name: "user", PassWd: "e10adc3949ba59abbe56e057f20f883e"},
		}
...
func createInitialBlogs(db *gorm.DB) error {
	...
	if count == 0 {
		blogs := []Blog{
			...
			{UserId: 2, Title: "博客标题3", Article: "博客内容3"},
			{UserId: 2, Title: "博客标题4", Article: "博客内容4"},
			{UserId: 2, Title: "博客标题5", Article: "博客内容5"},
		}
